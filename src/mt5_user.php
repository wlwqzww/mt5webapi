...
class MTUserProtocol
{
    ...
    private function CheckNull(&$user)
    {
        ...
        if ($user->CertSerialNumber == null) $user->CertSerialNumber = "";
        //--- registration datetime (filled by MT5)
        if ($user->Registration == null) $user->Registration = "";
        if ($user->LastAccess == null) $user->LastAccess = "";
        ...
        //--- leverage
        if ($user->Leverage == null) $user->Leverage = "";
        //--- agent account
        if ($user->Agent == null) $user->Agent = "";
        //--- main password
        ...
        if ($user->CommissionDaily == null) $user->CommissionDaily = "";
        if ($user->CommissionMonthly == null) $user->CommissionMonthly = "";
        //--- previous balance state
        if ($user->BalancePrevDay == null) $user->BalancePrevDay = "";
        if ($user->BalancePrevMonth == null) $user->BalancePrevMonth = "";
        //--- previous equity state
        if ($user->EquityPrevDay == null) $user->EquityPrevDay = "";
        if ($user->EquityPrevMonth == null) $user->EquityPrevMonth = "";
...
class MTUser
{
    ...
    public static function CreateDefault()
    {
        $user = new MTUser();
        //---
        $user->Rights = MTEnUsersRights::USER_RIGHT_ENABLED | MTEnUsersRights::USER_RIGHT_PASSWORD | MTEnUsersRights::USER_RIGHT_TRAILING | MTEnUsersRights::USER_RIGHT_EXPERT | MTEnUsersRights::USER_RIGHT_API | MTEnUsersRights::USER_RIGHT_REPORTS;
        $user->Leverage = 100;
