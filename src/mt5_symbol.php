...
class MTSymbolAnswer
{
    ...
    public function GetFromJson()
    {
        ...
        $result->Color = (string)$obj->Color;
        $result->ColorBackground = (string)$obj->ColorBackground;
        ...
        //---
        $result->OrderFlags = (float)$obj->OrderFlags;
...
class  MTEnCalcMode
{
    ...
    const TRADE_MODE_LAST = MTEnCalcMode::TRADE_MODE_EXCH_OPTIONS;
}
...
class  MTEnTickFlags
{
    ...
    //--- flags borders
    const TICK_NONE = 0;
    const TICK_ALL = MTEnTickFlags::TICK_REALTIME | MTEnTickFlags::TICK_COLLECTRAW | MTEnTickFlags::TICK_FEED_STATS;
}
 ...
 * RFlags trade orders
 */
class MTEnTradeOrderFlags
{
    const ORDER_FLAGS_NONE = 0; // all disabled
    const ORDER_FLAGS_MARKET = 1; // ascees market orders
    const ORDER_FLAGS_LIMIT = 2; // ascees limit orders
    const ORDER_FLAGS_STOP = 4; // ascees stop orders
    const ORDER_FLAGS_STOP_LIMIT = 8; // ascees limit orderразрешены стоп-лимит ордера
    const ORDER_FLAGS_SL = 16; // ascees limit orderразрешены стоп-лоссы
    const ORDER_FLAGS_TP = 32; // разрешены тейк-профиты
    //--- all
    const ORDER_FLAGS_FIRST = MTEnTradeOrderFlags::ORDER_FLAGS_MARKET;
    const ORDER_FLAGS_ALL = 63; // ORDER_FLAGS_MARKET|ORDER_FLAGS_LIMIT|ORDER_FLAGS_STOP|ORDER_FLAGS_STOP_LIMIT|ORDER_FLAGS_SL|ORDER_FLAGS_TP
}
