    ...
    private function GetParams($group)
    {
        if (!empty($group->Symbols)) {
            foreach ($group->Symbols as &$groupSymbol) {
                ...
                if ($groupSymbol->OrderFlags == MTEnTradeOrderFlags::ORDER_FLAGS_NONE) $groupSymbol->OrderFlags = 'default';
                //---
                ...
                if ($groupSymbol->MarginInitial == MTConGroupSymbol::DEFAULT_VALUE_UINT) $groupSymbol->MarginInitial = 'default';
...
//---
    ...
    {
        $data = array(MTProtocolConsts::WEB_PARAM_NAME => $name);
        //--- send request
...
class MTGroupAnswer
{
    ...
    public function GetFromJson()
    {
        ...
        foreach ($obj->Symbols as $symbol) {
            ...
            $s->OrderFlags = $symbol->OrderFlags == 'default' ? MTEnTradeOrderFlags::ORDER_FLAGS_NONE : (int)$symbol->OrderFlags;
            ...
            $s->REFlags = $symbol->REFlags == 'default' ? MTConGroupSymbol::GetDefault('reflags') : (float)$symbol->REFlags;
            $s->RETimeout = $symbol->RETimeout == 'default' ? MTConGroupSymbol::GetDefault('retimeout') : (float)$symbol->RETimeout;
            $s->IEFlags = $symbol->IEFlags == 'default' ? MTConGroupSymbol::GetDefault('ieflags') : (float)$symbol->IEFlags;
            $s->IECheckMode = $symbol->IECheckMode == 'default' ? MTConGroupSymbol::GetDefault('iecheckmode') : (float)$symbol->IECheckMode;
            $s->IETimeout = $symbol->IETimeout == 'default' ? MTConGroupSymbol::GetDefault('ietimeout') : (float)$symbol->IETimeout;
            $s->IESlipProfit = $symbol->IESlipProfit == 'default' ? MTConGroupSymbol::GetDefault('ieslipprofit') : (float)$symbol->IESlipProfit;
            $s->IESlipLosing = $symbol->IESlipLosing == 'default' ? MTConGroupSymbol::GetDefault('iesliplosing') : (float)$symbol->IESlipLosing;
...
class MTEnAuthMode
{
    ...
    const AUTH_RSA_CUSTOM = 4; // RSA custom
    //--- enumeration borders
    const AUTH_FIRST = MTEnAuthMode::AUTH_STANDARD;
    const AUTH_LAST = MTEnAuthMode::AUTH_RSA2048;
}
...
//--- symbol or symbol groups path
    ...
    public static function CreateDefault()
    {
        ...
        $groupSymbol->OrderFlags = MTEnTradeOrderFlags::ORDER_FLAGS_NONE;
